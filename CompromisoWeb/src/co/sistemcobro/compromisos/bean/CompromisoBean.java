package co.sistemcobro.compromisos.bean;
...
import co.sistemcobro.compromisos.constante.Constante;
...
import co.sistemcobro.compromisos.session.LoginBean;
import co.sistemcobro.compromisos.util.JasperReportUtil;
import co.sistemcobro.compromisos.util.NumeroLetras;
...
public class CompromisoBean implements Serializable {
	 ...
	 * los datos al report . 2018-12-14
	 * 
	 * @author Andres Diaz
	 * @param campos
	 *            provenientes de xhtml
	...
	public void generateReport() throws Exception {
		...
		try {
			...
			String logoTigo = FacesContext.getCurrentInstance().getExternalContext()
					.getRealPath(Constante.CONSTANTE_LOGO_TIGO);

			logger.info("el logo de tigo esta en... " + logoTigo);
			String logoSistemcobro = FacesContext.getCurrentInstance().getExternalContext()
					.getRealPath(Constante.CONSTANTE_LOGO_SISTEM);
			
			logger.info("el logo de logoSistemcobro esta en... " + logoSistemcobro);
...
//				parametro.put("Id_acuerdo", Integer.toString(consecutivo));
				...
				parametro.put("LOGOTIGO", logoTigo);
				parametro.put("LOGOSISTEM", logoSistemcobro);
